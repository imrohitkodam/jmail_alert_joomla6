								...
								foreach ($this->items as $i => $item)
								{
									...
									<tr class="row<?php echo $i % 2; ?>">
										...
										<td>
											...
											if ($item->state)
											{
												$link = Route::_('index.php?option=com_jmailalerts&task=subscriber.preview&tmpl=component&send_mail_to_box=admin@admin.com&flag=1&userid=&user_id=' . $item->user_id . '&select_date_box=' . $item->date . '&alert_id=' . $item->alert_id . '&email_id=' . rawurlencode($item->email_id));

												echo HTMLHelper::_(
													'bootstrap.renderModal',
													'preview' . (int) $item->id . 'Modal',
													array(
														'url'         => $link,
												...
												<a id ="<?php echo (int) $item->id; ?>"
													onclick="document.getElementById('preview<?php echo (int) $item->id; ?>Modal').open();"
